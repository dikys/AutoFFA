...
### Коэффициент очков силы (настраиваемая опция)

Эта механика добавляет новый тактический слой в сражения. У каждого игрока появляется **коэффициент очков силы**, который отображается под его статусом ("Сюзерен" или "Вассал").

*   **Как это работает**: Когда вы атакуете игрока, количество очков силы, которые вы получаете, умножается на его коэффициент. Например, атака по игроку с коэффициентом 2.0 принесет вам в два раза больше очков.
*   **Расчет коэффициента**:
    *   **Базовое значение**: У Сюзеренов базовый коэффициент — **1.0**, а у Вассалов — **0.5**. Это делает атаки на вассалов менее выгодными, защищая их.
    *   **Бонус за очки силы**: Коэффициент увеличивается на 10% за каждые 100% от начального количества очков силы. Это делает богатых и влиятельных игроков более желанной целью.
*   **Тактическое значение**: Эта механика заставляет игроков выбирать цели более осознанно. Атаковать сильного игрока рискованно, но награда может быть огромной. В то же время, "фармить" слабых вассалов становится менее эффективно.
...
**20260106**
*   **(Интерфейс)** Улучшена читаемость текстовых декораторов (Сила, Цель и т.д.). Теперь для их отрисовки используется осветленная версия цвета игрока, что делает текст более заметным на любом фоне.
*   **(Механика)** Добавлена новая механика "Коэффициент очков силы". Теперь у каждого игрока есть коэффициент, который влияет на количество очков, получаемых за атаку на него. У вассалов базовый коэффициент 0.5, у сюзеренов - 1.0. Коэффициент также растет в зависимости от текущего количества очков силы игрока. Это делает богатых игроков более ценными целями.
*   **(Интерфейс)** Коэффициент очков силы отображается под статусом игрока (Сюзерен/Вассал).
*   **(Баланс)** При получении урона, жертва теперь теряет 10% от очков силы, полученных атакующим.

**20260105**
*   **(Механика)** Улучшена механика `enableTeamBalancing`. Теперь перед балансировкой в чат отправляется предупреждение, а время до балансировки зависит от размера карты, давая игрокам время подготовиться.
*   **(Баланс)** Опция `enableTeamBalancing` теперь по умолчанию отключена (`false`).
*   **(Стабильность)** Улучшен алгоритм выбора точки для спавна замка, чтобы исключить появление в заблокированных или недоступных местах.
